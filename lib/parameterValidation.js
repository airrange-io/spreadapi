 ...
 * - undefined defaultValue (old services, preserve original behavior)
 * - NULL_DEFAULT_VALUE (new services with empty cell, clear the cell)
 * - actual value (new services with value, use that value)
 */
...
export function validateParameters(inputs, inputDefinitions) {
  ...
  for (const inputDef of inputDefinitions) {
    ...
    if (providedValue === undefined || providedValue === null || providedValue === '') {
      // Try lowercase exact match, title match, and variations
      for (const [key, value] of Object.entries(inputs)) {
        const keyLower = key.toLowerCase();
        const keyNoUnderscore = keyLower.replace(/[\s_-]/g, '');

        // Match by name (with/without underscores)
        if (keyLower === paramNameLower || keyNoUnderscore === paramNameNoUnderscore) {
          providedValue = value;
          break;
        }

        // Match by title (exact or without spaces/underscores/dashes)
        if (paramTitleLower && (keyLower === paramTitleLower || keyNoUnderscore === paramTitleNoUnderscore)) {
          providedValue = value;
          break;
        }
      }
  ...
  for (const [key, value] of Object.entries(inputs)) {
    ...
    const inputDef = inputDefinitions.find(
      (apiInput) => {
        ...
        return apiName === inputKey ||
               ...
               apiTitleNoUnderscore === inputKeyNoUnderscore;
      }
    );
    ...
    // Skip validation for optional parameters with empty values
    ...
    if (!validation.valid) {
      // Enrich with the canonical name + title so error formatters can tell the
      // AI the exact input KEY to fix (not just the human-readable title).
  ...
  for (const inputDef of inputDefinitions) {
    ...
    if (!isMandatory && hasDefaultValue) {
      ...
      if (providedValue === undefined || providedValue === null || providedValue === '') {
        ...
        // Pass it through without validation
        if (inputDef.defaultValue === NULL_DEFAULT_VALUE) {
          result[key] = NULL_DEFAULT_VALUE;
          console.log(`[Validation] Applied NULL default for ${key} (cell will be cleared)`);
        } else {
