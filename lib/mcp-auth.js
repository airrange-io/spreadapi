 ...
 */
export async function createToken(userId, name, description = '') {
...
export async function validateToken(token) {
  ...
  if (!tokenData || !tokenData.isActive || tokenData.isActive !== 'true') {
  ...
  for (const tokenId of tokenIds) {
    ...
    if (tokenData && tokenData.isActive === 'true') {
      tokens.push({
        ...
        fullToken: tokenId,  // Store full token for revoke
        name: tokenData.name,
