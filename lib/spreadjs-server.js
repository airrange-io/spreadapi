...
function initializeSpreadJS() {
  ...
  const globalProperties = {
    ...
    self: global,
    HTMLCollection: mockWindow.HTMLCollection,
    getComputedStyle: mockWindow.getComputedStyle,
    customElements: null,
    canvas: canvas,
    HTMLElement: mockWindow.HTMLElement,
    HTMLDivElement: mockWindow.HTMLDivElement
  };
  
  for (const [key, value] of Object.entries(globalProperties)) {
    try {
      if (Object.getOwnPropertyDescriptor(global, key)?.configurable !== false) {
        global[key] = value;
      } else {
      ...
      // If we can't set it, try to continue anyway
      console.warn(`Could not set global.${key}:`, e.message);
    }
  }
