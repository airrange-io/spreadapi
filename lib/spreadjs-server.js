...
let excelIoLoaded = false;

/**
 * Load the Excel I/O module (@mescius/spread-sheets-io) on demand.
 * This augments Workbook.prototype with .export()/.import(). It is purely
 * additive — it does not touch calculation — and is only loaded the first
 * time an export is requested, so the normal calc path is unaffected.
 */
function loadExcelIOModule() {
  if (excelIoLoaded) {
    return true;
  }
  ...
  try {
    ...
    excelIoLoaded = true;
    console.log('[SpreadJS] Excel I/O module loaded successfully');
    return true;
  } catch (error) {
    console.error('Failed to load Excel I/O module:', error);
    return false;
  }
}

// Default export options — mirror the client-side workbookManager so a
// server-generated file matches what the editor produces.
...
// Guard against a hung export holding a serverless function open indefinitely.
const XLSX_EXPORT_TIMEOUT_MS = 30 * 1000;

/**
 * Export a workbook instance to an .xlsx file, server-side.
 * @param {object} spread - a SpreadJS Workbook instance (already populated/calculated)
 * @param {object} [options] - SpreadJS export options (merged over defaults)
 * @returns {Promise<Buffer>} the .xlsx file as a Node Buffer
 */
function exportWorkbookToXlsx(spread, options = {}) {
  if (!spread || typeof spread.export !== 'function') {
    if (!loadExcelIOModule() || typeof spread.export !== 'function') {
      return Promise.reject(new Error('Excel I/O module not available'));
    }
  }

  const exportOptions = { ...DEFAULT_XLSX_EXPORT_OPTIONS, ...options };

  return new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error('Excel export timed out')),
      XLSX_EXPORT_TIMEOUT_MS
    );
    ...
    spread.export(
      ...
      (err) => {
        clearTimeout(timer);
        reject(new Error('Excel export failed: ' + (err && (err.errorMessage || err.message) || 'unknown error')));
      },
      exportOptions
    );
  });
