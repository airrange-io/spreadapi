...
export async function deleteUser(userId) {
  ...
  try {
    ...
    for (const serviceId of serviceIds) {
      ...
      // Delete all cache keys for this service
      multi.del(`service:${serviceId}:cache:api`);        // API definition cache
      multi.del(`service:${serviceId}:cache:workbook`);   // Workbook cache
      multi.del(`service:${serviceId}:cache:results`);    // Result cache hash
    }
