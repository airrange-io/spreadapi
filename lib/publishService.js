...
import { getLicenseType, getLimits } from './licensing';
...
export async function createOrUpdateService(apiId, publishData, tenant = null, userId = null) {
  ...
  try {
    ...
    // Validate that the workbook has real content (not just the default empty Sheet1)
    if (fileJson?.sheets) {
      const sheetNames = Object.keys(fileJson.sheets);
      if (sheetNames.length === 1 && sheetNames[0] === 'Sheet1') {
        // Check if any input/output references a different sheet
        const allParams = [...(apiJson.inputs || []), ...(apiJson.outputs || [])];
        const referencedSheets = new Set(
          allParams
            .map(p => (p.address || p.cell)?.split('!')[0]?.replace(/'/g, ''))
            .filter(Boolean)
        );
        const hasExternalRefs = [...referencedSheets].some(s => s !== 'Sheet1');
        if (hasExternalRefs) {
          throw new Error(
            `Workbook only contains Sheet1 but parameters reference sheets: ${[...referencedSheets].join(', ')}. ` +
    ...
    const publishedData = {
      ...
      // (token:{id} Redis hashes) via validateServiceToken in calculateDirect.js
      useCaching: apiJson.flags?.useCaching || "true",
      needsToken: apiJson.flags?.needsToken || "false",
      allowExcelExport: apiJson.flags?.allowExcelExport || "false",
    ...
    // Free plan: published services are ephemeral. Set a Redis TTL on the
    // :published key so the service auto-unpublishes (the execute/MCP/web-app
    // paths all gate on this key existing). Mirror the deadline onto the
    // persistent draft hash as `expiresAt` so the UI and the 402 "expired"
    // path can read it after the :published key is gone.
    //
    // Strictly gated on license: paid plans (publishTtlSeconds === null) get NO
    // TTL — and we defensively persist()/clear in case a stale TTL exists (e.g.
    // a free user who just upgraded re-publishes). Wrapped so a TTL failure can
    // never break the publish itself.
    if (userId && userId !== 'default') {
      try {
        const licenseType = getLicenseType(await redis.hGet(`user:${userId}`, 'licenseType'));
        const ttlSeconds = getLimits(licenseType).publishTtlSeconds;
        if (ttlSeconds) {
          const expiresAt = Date.now() + ttlSeconds * 1000;
          await redis.expire(`service:${apiId}:published`, ttlSeconds);
          await redis.hSet(`service:${apiId}`, { expiresAt: String(expiresAt) });
        } else {
          await redis.persist(`service:${apiId}:published`);
          await redis.hDel(`service:${apiId}`, 'expiresAt');
        }
      } catch (ttlError) {
        console.error(`[Publish] Failed to apply publish TTL for ${apiId}:`, ttlError);
      }
    }
...
export async function deleteService(apiId) {
  try {
    ...
    await redis.del(CACHE_KEYS.workbookCache(apiId));
    ...
    try {
      for await (const key of redis.scanIterator({ MATCH: `service:${apiId}:*`, COUNT: 100 })) {
        await redis.del(key);
      }
    } catch (scanError) {
