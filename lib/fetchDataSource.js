...
export async function fetchDataSource(source, opts = {}) {
  ...
  // Pipedream sources short-circuit the URL-based path entirely — they don't
  // have an upstream URL. Auth and the HTTP hop live inside Pipedream Connect.
  if (type === 'pipedream') {
    return fetchPipedreamSource(source, opts);
  }
...
// ────────────────────────────────────────────────────────────────────────────
// Pipedream Connect path. Keeps the same return contract as URL-based sources
// (`{ ok, rows, columns, totalRowsFetched }` on success, `{ ok:false, error,
// stage }` on failure) so the refresh endpoint and calculateDirect see a
// uniform result regardless of source type.
// ────────────────────────────────────────────────────────────────────────────
async function fetchPipedreamSource(source, opts = {}) {
  const {
    appSlug,
    actionId,
    accountId,
  ...
  if (!appSlug || !actionId || !accountId || !externalUserId) {
    return {
      ok: false,
      stage: 'validate',
      error: 'Pipedream source is incomplete (appSlug, actionId, accountId, externalUserId all required)',
    };
  }

  let makePipedreamClient;
  let normalizeActionResult;
  let getByPath;
  let getProvider;
  try {
    ({ makePipedreamClient } = require('./pipedream/client'));
    ({ normalizeActionResult, getByPath } = require('./pipedream/normalize'));
    ({ getProvider } = require('./pipedream/providers'));
  } catch (e) {
    return {
      ok: false,
      stage: 'validate',
      error: 'Pipedream integration is not available: ' + (e?.message || 'unknown'),
    };
  }

  let pd;
  try {
    pd = makePipedreamClient();
  } catch (e) {
    return {
      ok: false,
      stage: 'validate',
      error: e?.message || 'Pipedream is not configured',
    };
  }

  const provider = getProvider(appSlug);
  const maxRows = Math.max(1, opts.maxRows || 5000);

  const runOne = async (props) => {
    const result = await pd.actions.run({
      id: actionId,
      externalUserId,
      configuredProps: {
        [appSlug]: { authProvisionId: accountId },
        ...props,
      },
    });
    return { ret: result?.ret, exports: result?.exports, os: result?.os };
  };

  try {
    let rows;
    if (typeof provider.customFetch === 'function') {
      // Bypass actions.run entirely — the provider handles the fetch itself
      // (usually via pd.proxy.get against the vendor API).
      rows = await provider.customFetch({ pd, source, maxRows });
    } else if (typeof provider.paginate === 'function') {
      rows = await provider.paginate(runOne, configuredProps, maxRows);
    } else {
      const result = await runOne(configuredProps);
      const ret = result.ret ?? result.exports ?? null;
      if (arrayPath) {
        const found = getByPath(ret, arrayPath);
        rows = Array.isArray(found) ? found : [];
      } else if (typeof provider.normalizeResponse === 'function') {
        rows = provider.normalizeResponse(ret);
      } else {
        rows = normalizeActionResult(ret);
      }
    }

    if (!Array.isArray(rows)) rows = [];
    const totalRowsFetched = rows.length;
    const capped = rows.slice(0, maxRows);
    const schema = inferSchema(capped);
    return { ok: true, rows: capped, columns: schema, totalRowsFetched };
  } catch (e) {
    const isAuth =
      e?.statusCode === 401 ||
      /unauth|revoked|invalid.+token|not.+authoriz/i.test(e?.message || '');
    return {
      ok: false,
      stage: isAuth ? 'auth' : 'fetch',
      error: isAuth
        ? 'Connected account is no longer authorized. Reconnect in the data source editor.'
        : e?.message || 'Pipedream action failed',
    };
  }
}
