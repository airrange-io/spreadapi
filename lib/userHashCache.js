...
export async function cacheUserData(userId, userData) {
  ...
  try {
    ...
    // Set expiration
    pipeline.expire(userKey, USER_CACHE_TTL);
    
    // Add to user index (for admin purposes)
    pipeline.sAdd('users:index', userId);
...
export async function updateUserCache(userId, updates) {
  ...
  try {
    ...
    for (const [field, value] of Object.entries(updates)) {
      ...
      pipeline.hSet(userKey, field, stringValue);
    }
    
    // Update lastActivity
    pipeline.hSet(userKey, 'lastActivity', new Date().toISOString());
