...
class ServiceAI {
  ...
  async _generateFinalSuggestions(context, conversationHistory) {
    ...
    const suggestionSchema = z.object({
      ...
      parameterDescriptions: z.array(z.object({
        name: z.string().describe('The EXACT parameter name as listed in the service context above.'),
        direction: z.enum(['input', 'output']).describe('Whether this is an input or output parameter.'),
        description: z.string().describe('A concise (1-2 sentence) AI-facing description of THIS specific parameter — its meaning, unit/scale, sign or format convention, or how to interpret an output value. Only what the structured schema (name/type/range/allowed values) cannot already express.')
      })).describe('One entry per input and output parameter listed in the service context.'),
...
- parameterDescriptions: for EACH input and output parameter listed above, a short (1-2 sentence) description of what THAT specific parameter means or how to interpret it — units, sign/format conventions, what an output value represents, relationships to other parameters. Use the EXACT parameter name and the correct direction (input/output). Do NOT restate the type, range or allowed values (the calling AI already has those); if a parameter is fully self-explanatory from its name, keep it to a single short clause. Do NOT invent parameters that are not listed above.
