...
class ServiceAI {
  ...
  async generateAIInfo(conversationHistory = []) {
    ...
    // First, check if we need to ask clarifying questions
    if (conversationHistory.length === 0) {
      // Initial request - analyze if we need more information
      const needsQuestions = await this._checkIfNeedsQuestions(context);
    ...
    // Either we have conversation history or no questions needed - generate final suggestions
