...
export async function executeEnhancedCalc(serviceId, inputs = {}, areaUpdates = [], returnOptions = {}, auth) {
  ...
  try {
    ...
    // needsToken gate is satisfied for protected services (the MCP auth layer has
    // already validated it upstream) — otherwise this throws for token services.
    const apiData = await getApiDefinition(serviceId, auth);
    if (apiData.error) {
