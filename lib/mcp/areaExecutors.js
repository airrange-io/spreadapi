...
const { createWorkbook, getCachedWorkbook } = require('../spreadjs-server');
...
export async function executeAreaRead(serviceId, areaName, options, auth) {
  try {
    // Get service definition. Pass the caller's token (auth) so getApiDefinition's
    // needsToken gate is satisfied for protected services (MCP auth already
    // validated it upstream) — otherwise this throws for token services.
    const apiDefinition = await getApiDefinition(serviceId, auth);
    ...
    const fileJsonSizeBytes = JSON.stringify(fileJson).length;
...
export async function executeAreaUpdate(serviceId, updates, auth, returnOptions = {}) {
  try {
    ...
    spread.fromJSON(fileJson, {
      calcOnDemand: false,
      doNotRecalculateAfterLoad: true,  // Performance optimization: Skip initial calc, calculate after setting values
    });
