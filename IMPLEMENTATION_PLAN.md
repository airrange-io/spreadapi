...
export function generateOpenAPISpec(
  ...
  const spec = {
    ...
    security: definition.requiresToken ? [{ bearerAuth: [] }] : [],
    paths: {
      [`/api/v1/services/${serviceId}/execute`]:
        generateExecuteEndpoint(definition),
      [`/api/v1/services/${serviceId}/definition`]:
        generateDefinitionEndpoint(),
    },
    components: {
...
function generateExecuteEndpoint(definition: RedisServiceDefinition) {
  return {
    get: {
      ...
      description:
        "Execute spreadsheet calculation via GET request. Ideal for Excel integration, simple browser calls, and testing.",
    ...
    post: {
      ...
      description:
        "Execute spreadsheet calculation via POST request. Recommended for production integrations.",
...
function generateDefinitionEndpoint() {
  return {
    get: {
      ...
      description:
        "Retrieve complete API definition including validation rules, input/output schemas, and AI metadata",
...
function generateExampleRequest(definition: RedisServiceDefinition) {
  ...
  definition.inputs.forEach((input) => {
    if (input.aiExamples && input.aiExamples.length > 0) {
      ...
      switch (input.type) {
        case "number":
          inputs[input.name] = 100;
          break;
        case "currency":
          inputs[input.name] = 10000;
          break;
        case "percentage":
          inputs[input.name] = 0.05;
          break;
        case "boolean":
...
export async function GET() {
  const spec = {
    ...
    info: {
      ...
      version: "1.0.0",
      description:
        "Convert spreadsheets into powerful APIs. Execute complex calculations, validations, and business logic through RESTful endpoints.",
      contact: {
        name: "SpreadAPI Support",
...
describe("OpenAPI Specification Generation", () => {
  ...
  test("maps input validation rules correctly", () => {
    ...
    const getParams =
      spec.paths["/api/v1/services/test-service/execute"].get.parameters;
  ...
  try {
    ...
    for (const inputDef of inputDefs) {
      ...
      if (
        ...
        inputDef.type === "currency" ||
        inputDef.type === "percentage"
      ) {
    ...
    const results = await Promise.allSettled(
      body.requests.map(async (req: any, index: number) => {
        try {
          ...
          const result = await calculateDirect(serviceId, req.inputs, token, {
